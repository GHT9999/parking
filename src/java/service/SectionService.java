...
public class SectionService implements IDao<Section> {
    ...
    public boolean update(Section o) {
       String sql = "update section set reference = ? where id  = ?";
        try {
            PreparedStatement ps = Connexion.getInstane().getConnection().prepareStatement(sql);
            ps.setString(1, o.getReference());
            ps.setInt(2, o.getId());
    ...
    String sql = "select * from section where id  = ?";
        try {
            ...
            if (rs.next()) {
                return new Section(rs.getInt("id"), rs.getString("reference"));
    ...
    List<Section> Sections = new ArrayList<Section>();
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                Sections.add(new Section(rs.getInt("id"), rs.getString("reference")));
            }
